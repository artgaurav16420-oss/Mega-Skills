...
1. Reduce colors (128 → 64 colors)

1. Reduce dimensions (480x480 → 320x320)
...
1. Use 32-40 colors maximum

1. Avoid gradients (solid colors compress better)

1. Simplify design (fewer elements)

1. Use `optimize_for_emoji=True` in save method
...
1. **Design the animation** - Break it into phases (anticipation, action, reaction)

1. **Apply primitives as needed** - Shake, bounce, move, effects - mix freely

1. **Validate constraints** - Check file size, especially for emoji GIFs
