...
[![CI]([https://github.com/artgaurav16420-oss/Mega-Skills/actions/workflows/ci.yml/badge.svg](https://github.com/artgaurav16420-oss/Mega-Skills/actions/workflows/ci.yml/badge.svg))]([https://github.com/artgaurav16420-oss/Mega-Skills/actions/workflows/ci.yml)](https://github.com/artgaurav16420-oss/Mega-Skills/actions/workflows/ci.yml))
...
1. **🛠️ Core Engineering**: `brainstorming` • `writing-plans` • `test-driven-development` • `sequential-thinking` • `systematic-debugging` • `using-git-worktrees` • `memory-management`
1. **🏗️ Developer Tools & DevOps**: `mcp-builder` • `browser-automation` • `webapp-testing` • `claude-api` • `web-artifacts-builder` • `langsmith-fetch` • `changelog-generator`
1. **📄 Document & Enterprise**: `pdf` • `xlsx` • `docx` • `pptx` • `invoice-organizer` • `file-organizer` • `internal-comms`
1. **🎨 Design & Content Art**: `frontend-design` • `canvas-design` • `slack-gif-creator` • `twitter-algorithm-optimizer` • `youtube-downloader` • `theme-factory` • `algorithmic-art`
...
1. **Discord**: [Join the Mega-Skills Community]([https://discord.gg/35wsABTejz)](https://discord.gg/35wsABTejz))
1. **Sponsorship**: Help us scale the library by [sponsoring Gaurav's work]([https://github.com/artgaurav16420-oss).](https://github.com/artgaurav16420-oss).)
1. **Contributing**: Read [CONTRIBUTING.md](CONTRIBUTING.md). We maintain a 94% rejection rate to ensure only the highest fidelity skills enter the core library.
